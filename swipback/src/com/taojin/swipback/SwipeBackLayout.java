...
public class SwipeBackLayout extends FrameLayout {
    ...
    private static final int DEFAULT_SCRIM_COLOR = Color.TRANSPARENT;
    ...
    private OnPageScrollEndCallBack onPageScrollEndCallBack;
    ...
    public void setOnPageScrollEndCallBack(OnPageScrollEndCallBack onPageScrollEndCallBack) {
        this.onPageScrollEndCallBack = onPageScrollEndCallBack;
    }
...
//        Log.d("SwipeBack","mContentView="+(mContentView==null?null:"不为null")+"  mScrollPercent=="+mScrollPercent);
        if (mContentView == null) return;
        if (mScrollPercent >= 1) {//这句不能少，以前经常碰到页面点击没反应，就是因为有时候没有回调
...
//            Log.d("SwipeBack","mScrollPercent=="+mScrollPercent);
            if (mScrollPercent >= 1) {
                //当mScrollPercent>=1时候，页面已经滚动到最后这是页面应该调用finish，但我们是应该统一调用onBackPress，所以这里用回调
                if (onPageScrollEndCallBack != null) {
                    onPageScrollEndCallBack.onPageScrollEnd();
                } else {
//                    Log.d("SwipeBack","mActivity.isFinishing()=="+mActivity.isFinishing());
                    if (!mActivity.isFinishing()) {
                        mActivity.finish();
                    }
    ...
    /**
     * 当mScrollPercent>=1时候，页面已经滚动到最后这是页面应该调用finish，但我们是统一调用onBackPress
     * 包括setResult的时候，所以这里用回调，否则我们之前写在onbackPress里面的逻辑代码都无效
     */
    public interface OnPageScrollEndCallBack {
