...
public class DangqianweituoAdapter extends BaseLoadMoreImageLoaderRecycleAdapter<Position> {
    ...
    public DangqianweituoAdapter(Context context) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolderWithoutFoot(ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(context).inflate(R.layout.item_dangqianweituo, parent, false));
    }
        ...
        public void setData(final Position data, final int pos) {
            if (!TextUtils.isEmpty(data.buySell)){
                if (data.buySell.equals("buy")||data.buySell.equals("买入")){
                    tvBuySell.setText(data.symbol+"· 买入");
                }else {
                    tvBuySell.setText(data.symbol+"· 卖出");
                }
            }else {
                tvBuySell.setText("");
            }
    ...
    private void startOrderCancel(final Position data, final int pos, String payPass) {
        ...
        tradeCancelCall = VHttpServiceManager.getInstance().getVService().proOrderCancel2(data.orderId, payPass,"2");
