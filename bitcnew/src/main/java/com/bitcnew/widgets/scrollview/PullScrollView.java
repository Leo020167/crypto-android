package com.bitcnew.widgets.scrollview;
...
public class PullScrollView extends ScrollView {
    ...
    private boolean dragMode=true;//是否可以拖动
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if(dragMode){
            ...
            if(mOnScrollListener!=null){
                if(t>200){
                    mOnScrollListener.showActionBar();
                }else{
                    mOnScrollListener.hideActionBar();
                }
            }
        }
    }
    ...
    private void doActionMove(MotionEvent event) {
        ...
        if (isMoving&&mCurrentTop<=-15) {//防止被拖出头
            // 初始化content view矩形
            ...
            if (top <= headerBottom) {
                Log.d("doActionMove","mCurrentTop=="+mCurrentTop+"  mCurrentBottom=="+mCurrentBottom);
                // 移动content view
