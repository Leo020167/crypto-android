package com.bitcnew.widgets.swipecaptcha;
...
public class SwipeCaptchaView2 extends AppCompatImageView {
    private static final String TAG = "zxt/" + SwipeCaptchaView2.class.getName();
    //控件的宽高
...
//    private Bitmap mMaskShadowBitmap;
    ...
    public SwipeCaptchaView2(Context context) {
        this(context, null);
    }

    public SwipeCaptchaView2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SwipeCaptchaView2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void createMatchAnim() {
        ...
        mFailAnim.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                onCaptchaMatchCallback.matchFailed(SwipeCaptchaView2.this);
            }
        });
        ...
        mSuccessAnim.addListener(new AnimatorListenerAdapter() {
            ...
            @Override
            public void onAnimationEnd(Animator animation) {
                onCaptchaMatchCallback.matchSuccess(SwipeCaptchaView2.this);
...
//        mMaskShadowBitmap = mMaskBitmap.extractAlpha();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isMatchMode) {
            ...
            //绘制滑块
            // isDrawMask  绘制失败闪烁动画用
            if (null != mMaskBitmap  && isDrawMask) {
                // 先绘制阴影
//                canvas.drawBitmap(mMaskShadowBitmap, -mCaptchaX + mDragerOffset, 0, mMaskShadowPaint);
    ...
    public interface OnCaptchaMatchCallback {
        void matchSuccess(SwipeCaptchaView2 swipeCaptchaView);

        void matchFailed(SwipeCaptchaView2 swipeCaptchaView);
    }
     ...
     * @return
     */
    public SwipeCaptchaView2 setOnCaptchaMatchCallback(OnCaptchaMatchCallback onCaptchaMatchCallback) {
