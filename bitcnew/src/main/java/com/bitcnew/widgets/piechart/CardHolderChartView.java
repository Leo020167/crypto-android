package com.bitcnew.widgets.piechart;
...
import com.bitcnew.util.LinearEquationInTwoUnknowns;
import com.bitcnew.http.base.Group;
import com.bitcnew.module.myhome.entity.Trend;
...
public class CardHolderChartView extends View {
    ...
    private static final int lineColor = Color.parseColor("#3d3a50");
    ...
    public CardHolderChartView(Context context, AttributeSet attrs) {
        super(context, attrs);
        //创建初始化
        init(context);
    }
...
//        }
        ...
        if (minNum >= 0) {
            isMinus = false;
            countScale = (int) (maxNum / (maxCountScale - 1)) + 1;
            for (int i = 0, m = maxCountScale; i < m; i++) {
                count[i] = countScale * i;
            }
        } else {//有负数情况出现
...
//            Log.d("CardHolderChartView", "a==" + a);
            countScale = (int) (((Math.abs(maxNum) + Math.abs(minNum)) / (maxCountScale - 1)) * 2 + 1);
...
//            setBackgroundColor(Color.WHITE);
        ...
        for (int i = 0, m = count.length; i < m; i++) {
            ...
            canvas.drawText(String.valueOf(count[i]), x, y - textHeight / 2, mTextPaint);
...
//        画点和文字
        ...
        for (int i = 0, m = num.length; i < m; i++) {
            ...
            if (isMinus) {
                ...
                fy = (float) (mHeigth - (mHeigth - bottomHeight - topHeight) * Double.parseDouble(num[i]) / maxPoint - bottomHeight);
            }
...
//                Color.argb(50, 224, 224, 226),
                Color.argb(150, 97, 117, 174),
                Color.argb(50, 97, 117, 174),
                Shader.TileMode.CLAMP);
        mLinePaint.setShader(gradient);
