...
public class BadgeView extends AppCompatTextView {
    ...
    public static final int POSITION_VERTICAL_RIGHT = 6;

    private static final int DEFAULT_MARGIN_DIP = 5;
    private static final int DEFAULT_LR_PADDING_DIP = 5;
    private static final int DEFAULT_CORNER_RADIUS_DIP = 9;// 默认是8
    private static final int DEFAULT_POSITION = POSITION_TOP_RIGHT;
    private static final int DEFAULT_BADGE_COLOR = Color.parseColor("#6175ae"); // CCFF0000
    // Color.ZHANG;
    ...
    private void init(Context context, View target, int tabIndex) {
        ...
        badgeRadius = DEFAULT_CORNER_RADIUS_DIP;
        setTypeface(Typeface.DEFAULT_BOLD);
        setPadding(dipToPixels(5), dipToPixels(1), dipToPixels(5), dipToPixels(1));
    ...
    private void applyTo(View target) {
        ...
        if (target instanceof TabWidget) {
            ...
            ViewGroup.MarginLayoutParams m = (ViewGroup.MarginLayoutParams)target.getLayoutParams();
            m.setMargins(0, 0, 0, 0);// 在Android 7.0 的版本,这个 target 的MarginLayoutParams 不会被清零,旧版的会清零。
    ...
    private void show(boolean animate, Animation anim) {
        if (getBackground() == null) {
            if (badgeBg == null) {
                badgeBg = getDefaultBackground(badgeRadius);
            }
    ...
    private ShapeDrawable getDefaultBackground(int radius) {
     ...
     * @param badgeColor the badge background color.
     */
    public void setBadgeBackgroundColor(int badgeRadius, int badgeColor) {
        this.badgeRadius = badgeRadius;
        this.badgeColor = badgeColor;
        badgeBg = getDefaultBackground(badgeRadius);
    }

    public void setBadgeBackgroundRadis(int badgeRadius) {
        this.badgeRadius = badgeRadius;
        badgeBg = getDefaultBackground(badgeRadius);
    }
    ...
    public void setBadgeText(CharSequence text) {
        if (TextUtils.isEmpty(text)) {
            if (getContext() != null) {
                SpannableString spannable = new SpannableString("[new]");
                Drawable drawable = getResources().getDrawable(R.drawable.ic_home_new_c72e2fd_point);
                drawable.setBounds(0, 0, dipToPixels(8), dipToPixels(8));
                spannable.setSpan(new ImageSpan(drawable), 0, "[new]".length(), Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
                setText(spannable);
    ...
    /**
     * @param num
     * @param isPoint true 为显示数字 false 只显示红点
     */
    public void setViewRedPoint(int num, boolean isPoint) {
        if (num > 0) {
            if (isPoint) {
                if (num > 99) {
                    setBadgeText("99+");
                } else {
                    setBadgeText(String.valueOf(num));
                }
                show();
            } else {
                show();
