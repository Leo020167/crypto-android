package com.bitcnew.http.resource;
...
import com.bitcnew.http.common.CommonConst;

/**
 * 这个是下载图片时需要用到的类，主要管理接口
 * 
 * @author zhengmj
 * 
 */
public final class BaseRemoteResourceFetcher extends Observable {
	public static final String TAG = "RemoteResourceFetcher";
	public static final boolean DEBUG = CommonConst.DEBUG;
	...
	public BaseRemoteResourceFetcher(DiskCache cache) {
		...
		return new Callable<Request>() {
			public Request call() {
				try {
					...
					mResourceCache.store(request.uri.toString(), is);// 下载头像使用
	...
	public DiskCache getmResourceCache() {
